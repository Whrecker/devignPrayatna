...
def show_results(char):
    ...
    for i, ch_img in enumerate(char):
        ...
        # Make sure it has 3 color channels (depending on your model)
        img_ = fix_dimension(img_)
        # Reshape to (1, 28, 28, 3) for batch dimension
        img_ = img_.reshape(1, 28, 28, 3)
        
        # If using TF 2.6 or newer, do:
        prediction = model.predict(img_)
        y_ = np.argmax(prediction, axis=-1)[0]

        # If using an older TF version that supports predict_classes(), do:
        # y_ = model.predict_classes(img_)[0]
